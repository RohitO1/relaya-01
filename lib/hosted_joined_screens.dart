...
class HostedByYouScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _PremiumBackground(
        ...
        child: Scaffold(
          backgroundColor:
              isDoodleMode(context) ? DoodleColors.cream : Colors.transparent,
          body: ListView(
            ...
            children: [
              ...
              _CategoryCard(
                icon: Icons.flash_on,
                title: 'Rush-Ins',
                subtitle: 'Manage your live rush-in participants',
                color: _cyan,
              ...
              _CategoryCard(
                icon: Icons.event,
                title: 'Activities',
                subtitle: 'Manage your activity participants',
                color: _violet,
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
              ...
              _CategoryCard(
                ...
                title: 'Events',
                subtitle: 'Manage your event participants',
                color: _amber,
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
              ...
              _CategoryCard(
                ...
                title: 'Companionship',
                subtitle: 'Manage companion requests',
                color: _green,
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
...
class _HostedCategoryScreen extends StatefulWidget {
  ...
  @override
  State<_HostedCategoryScreen> createState() => _HCSState();
}
  ...
  Future<void> _fetchCreations() async {
    try {
      final all = await _sb
          .from('activities')
          .select(
              'id, title, description, location_name, activity_time, category, created_at, is_rush_in, expires_at, duration_hours, participant_limit')
          .eq('user_id', _uid)
          .order('created_at', ascending: false);
      final now = DateTime.now();
      final filtered = (all as List)
          .where((a) {
            final isRushIn = a['is_rush_in'] == true ||
                (a['description']?.toString().contains('[is_rush_in:true]') ??
                    false);
            if (widget.isRushIn) {
              ...
              if (expiresStr != null) {
                ...
                if (actStr != null) {
                  ...
                  if (actTime != null &&
                      actTime.add(Duration(hours: durHours)).isBefore(now))
                    return false;
                }
              }
              return true;
      ...
      child: Scaffold(
        ...
        body: _loading
            ...
            : _myCreations.isEmpty
                ? _emptyState(widget.icon,
                    'You haven\'t hosted any ${widget.title.toLowerCase()}s yet')
                : RefreshIndicator(
                    color: widget.color,
                      ...
                      itemBuilder: (_, i) {
                        if (i == 0) {
                          return Padding(
                            ...
                            child: Text(
                                'Your ${widget.title}s · ${_myCreations.length} hosted',
...
class _CreationCard extends StatefulWidget {
  ...
  const _CreationCard(
      ...
      required this.title});
  @override
  State<_CreationCard> createState() => _CCState();
}
...
class _CCState extends State<_CreationCard> {
  ...
  void _updateCountdown() {
    final actStr = widget.creation['activity_time'] as String? ??
        widget.creation['created_at'] as String?;
    ...
    final end = expStr != null
        ? DateTime.tryParse(expStr)?.toLocal()
        : start?.add(
            Duration(hours: widget.creation['duration_hours'] as int? ?? 6));
  ...
  Future<void> _loadCounts() async {
    try {
      ...
      final reqs =
          await _sb.from('requests').select('status').eq('target_id', id);
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: widget.color.withValues(alpha: hasPending ? 0.3 : 0.1)),
        ),
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Icon(Icons.chevron_right,
                  color: widget.color.withValues(alpha: 0.4), size: 20),
            ]),
          ),
  ...
  Widget _miniChip(IconData ic, String text, Color c) =>
...
class _ParticipantProfileCard extends StatefulWidget {
  ...
  const _ParticipantProfileCard(
      {required this.req, required this.color, required this.isPending});
  @override
  State<_ParticipantProfileCard> createState() => _PPCState();
}
...
class _PPCState extends State<_ParticipantProfileCard> {
  ...
  Future<void> _updateStatus(String newStatus) async {
    ...
    try {
      await _sb
          .from('requests')
          .update({'status': newStatus}).eq('id', widget.req['id']);
      ...
      if (newStatus == 'approved') {
        ...
        if (senderId != null) {
          NotificationService.sendNotification(
            ...
            payload: {
              'request_id': widget.req['id'],
              'target_id': widget.req['target_id']
            },
          );
        }
            ...
            child: Row(children: [
              ...
              if (_busy)
                const SizedBox(
                    ...
                    height: 24,
                    child:
                        CircularProgressIndicator(strokeWidth: 2, color: _cyan))
              else if (widget.isPending) ...[
                _actionBtn(Icons.check_rounded, _green, 'Approve',
                    () => _updateStatus('approved')),
                const SizedBox(width: 8),
                _actionBtn(Icons.close_rounded, _red, 'Reject',
                ...
                const SizedBox(width: 8),
                _actionBtn(Icons.undo_rounded, _amber, 'Revoke',
  ...
  Widget _actionBtn(IconData ic, Color c, String tooltip, VoidCallback onTap) =>
      Tooltip(
        message: tooltip,
        child: GestureDetector(
          onTap: onTap,
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (_, snap) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) => _ParticipantProfileCard(
              req: list[i], color: _green, isPending: status == 'pending'),
        );
      },
    );
  }
          ...
          body: ListView(
            ...
            children: [
              ...
              _CategoryCard(
                icon: Icons.flash_on,
                title: 'Rush-Ins',
                subtitle: 'Rush-ins you requested to join',
                color: _cyan,
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
              ...
              _CategoryCard(
                icon: Icons.event,
                title: 'Activities',
                subtitle: 'Activities you signed up for',
                color: _violet,
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
              ...
              _CategoryCard(
                icon: Icons.volunteer_activism,
                title: 'Companionship',
                subtitle: 'Companion requests you sent',
                color: _green,
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (_, snap) {
        ...
        final byType = (snap.data ?? []).where((r) {
          ...
          return r['target_type'] == 'activity' ||
              r['target_type'] == 'rush_in';
        }).toList();

        // For pending tab: just filter by status
        if (tabType == 'pending') {
          final pending =
              byType.where((r) => r['status'] == 'pending').toList();
        ...
        final approved =
            byType.where((r) => r['status'] == 'approved').toList();
...
class _JoinedEnrichedCard extends StatefulWidget {
  ...
  @override
  State<_JoinedEnrichedCard> createState() => _JECState();
}
...
class _JECState extends State<_JoinedEnrichedCard> {
  ...
  Future<void> _loadDetails() async {
    try {
      ...
      if (widget.filterRushIn && act['is_rush_in'] != true) {
        if (mounted) setState(() => _hidden = true);
        return;
      }
      if (!widget.filterRushIn &&
          !widget.filterEvent &&
          act['is_rush_in'] == true) {
        if (mounted) setState(() => _hidden = true);
        return;
      }
      if (widget.filterEvent && act['category'] != 'event') {
        if (mounted) setState(() => _hidden = true);
        return;
      }
      ...
      if (widget.tabType == 'past' || widget.tabType == 'upcoming') {
        ...
        if (timeStr != null) {
          ...
          if (actTime != null) {
            final now = DateTime.now();
            if (widget.tabType == 'past' && actTime.isAfter(now)) {
              if (mounted) setState(() => _hidden = true);
              return;
            }
            if (widget.tabType == 'upcoming' && actTime.isBefore(now)) {
              if (mounted) setState(() => _hidden = true);
              return;
            }
          }
      ...
      onTap: () {
        if (widget.targetType == 'companion') {
          Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (_) =>
                      ProfileScreen(userId: widget.req['target_id'])));
        } else {
          if (_activity != null) {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => RushInConsumerDetailView(
                        activity: _activity!, onInteraction: () {})));
          }
        }
      },
      child: Container(
          ...
          Container(
            ...
            child: Row(children: [
              CircleAvatar(
                radius: 14,
                backgroundImage:
                    hostAvatar != null ? NetworkImage(hostAvatar) : null,
                backgroundColor: widget.color.withValues(alpha: 0.15),
              ...
              const SizedBox(width: 8),
              Expanded(
                  child: Text('Hosted by $hostName',
                      style: GoogleFonts.inter(
                          color: Colors.white38, fontSize: 11))),
              Icon(Icons.arrow_forward_ios,
                  size: 12, color: widget.color.withValues(alpha: 0.5)),
            ]),
          ),
        ]),
      ),
