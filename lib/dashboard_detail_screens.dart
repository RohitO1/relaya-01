...
Color _getBg(BuildContext context) =>
    isDoodleMode(context) ? DoodleColors.cream : const Color(0xFF0A0A0F);
Color _getCard(BuildContext context) =>
    isDoodleMode(context) ? DoodleColors.paper : const Color(0xFF141C2E);
...
// ─── helpers ───────────────────────────────────────────────────────────────
PreferredSizeWidget _appBar(BuildContext context, String t,
    {List<Widget>? actions}) {
        ...
        body: StreamBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (_, snap) {
            final list = (snap.data ?? [])
                .where((r) => r['target_type'] == targetType)
                .toList();
            ...
            return ListView.builder(
              ...
              itemBuilder: (_, i) => _ParticipantRequestCard(
                  req: list[i],
                  color: color,
                  icon: icon,
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (_, snap) {
        ...
        final desc = act?['category'] as String? ??
            act?['description'] as String? ??
            '...'; // using category or desc as the subtext like 'football'
              ...
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (_) => RushInConsumerDetailView(
                          activity: act, onInteraction: () {})));
            }
          },
          child: Container(
            margin: const EdgeInsets.only(bottom: 12),
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
            decoration: isDoodleMode(context)
                ...
                : BoxDecoration(
                    ...
                    border: Border.all(
                        color: Colors.white
                            .withValues(alpha: 0.05)), // subtle border
                  ),
            child: Row(children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: const Color(0xFFFF6B00).withValues(
                      alpha: 0.1), // light blue tint like in screenshot
                  borderRadius: BorderRadius.circular(12),
                ),
                child: const Icon(Icons.radar,
                    color: Color(0xFFFF6B00),
                    size: 24), // @ icon style or radar
              ),
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                      color: sColor.withValues(
                          alpha: 0.5)), // slightly thicker outline
                ),
...
class ParticipantActivityDetailScreen extends StatefulWidget {
  ...
  final String targetType;
  const ParticipantActivityDetailScreen(
      {super.key, required this.request, required this.targetType});
  @override
  State<ParticipantActivityDetailScreen> createState() => _PADS();
}
...
class _PADS extends State<ParticipantActivityDetailScreen> {
  ...
  Future<void> _fetch() async {
    try {
      final act = await _sb
          .from('activities')
          .select('*')
          .eq('id', widget.request['target_id'])
          .maybeSingle();
      if (act != null) {
        if (mounted)
          setState(() {
  ...
  Widget build(BuildContext context) {
    if (_loading)
      return Scaffold(
          backgroundColor: _getBg(context),
          appBar: _appBar(context, 'Loading...'),
          body: const Center(child: CircularProgressIndicator(color: _cyan)));
    if (_act == null)
      return Scaffold(
          backgroundColor: _getBg(context),
          appBar: _appBar(context, 'Not Found'),
          body: _empty(Icons.error_outline, 'This event no longer exists'));
...
class HostActivityDetailScreen extends StatefulWidget {
  ...
  @override
  State<HostActivityDetailScreen> createState() => _HADS();
}
...
class _HostParticipantCard extends StatefulWidget {
  ...
  @override
  State<_HostParticipantCard> createState() => _HPCState();
}
...
class _HPCState extends State<_HostParticipantCard> {
  ...
  Future<void> _respond(String status) async {
    ...
    try {
      await _sb
          .from('requests')
          .update({'status': status}).eq('id', widget.req['id']);
    } catch (e) {
      if (mounted)
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        if (_busy)
          ...
          _btn(
              context, Icons.check_rounded, _green, () => _respond('approved')),
          const SizedBox(width: 8),
          _btn(context, Icons.close_rounded, _red, () => _respond('rejected')),
        ] else if (status == 'approved')
          _btn(context, Icons.close_rounded, _red, () => _respond('rejected'))
        else if (status == 'rejected')
          _btn(context, Icons.refresh_rounded, _green,
              () => _respond('approved')),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) => _StubScreen(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (_, snap) {
          ...
          if (list.isEmpty)
            return _empty(Icons.people_outline, 'No pending follow requests');
          return ListView.builder(
...
class _FollowRequestCard extends StatefulWidget {
  ...
  @override
  State<_FollowRequestCard> createState() => _FRCState();
}
...
class _FRCState extends State<_FollowRequestCard> {
  ...
  Future<void> _act(String s) async {
    setState(() => _busy = true);
    await _sb.from('requests').update({'status': s}).eq('id', widget.req['id']);
    if (mounted) setState(() => _busy = false);
  }

  @override
  Widget build(BuildContext ctx) {
    final av = _p?['avatar_url'] as String?;
    final nm = _p?['name'] as String? ?? 'User';
    return Container(
        ...
        child: Row(children: [
          CircleAvatar(
              radius: 22,
              backgroundImage: av != null ? NetworkImage(av) : null,
              backgroundColor: const Color(0xFF00FFFF).withValues(alpha: 0.15),
              child: av == null
                  ? Text(nm[0], style: const TextStyle(color: Colors.white))
                  : null),
          const SizedBox(width: 12),
          Expanded(
              child: Column(
          ...
          if (_busy)
            ...
            TextButton(
                ...
                child: const Text('Decline',
                    style: TextStyle(color: Color(0xFFFF4655)))),
          ],
        ]));
  }
}
...
class SavedCollectionsScreen extends StatefulWidget {
  ...
  @override
  State<SavedCollectionsScreen> createState() => _SCS();
}

class _SCS extends State<SavedCollectionsScreen> {
  List<Map<String, dynamic>> _savedActs = [];
  bool _loading = true;
  ...
  Future<void> _fetch() async {
    ...
    final ids = prefs.getStringList('saved_activities') ?? [];
    if (ids.isNotEmpty) {
      try {
        final res =
            await _sb.from('activities').select('*').inFilter('id', ids);
        if (mounted)
          setState(() => _savedActs = List<Map<String, dynamic>>.from(res));
      } catch (_) {}
    }
    if (mounted) setState(() => _loading = false);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return _StubScreen(
        ...
        body: ListView.builder(
          padding: const EdgeInsets.all(16),
          itemCount: _savedActs.length,
          itemBuilder: (_, i) {
            final act = _savedActs[i];
            final title = act['title'] as String? ?? 'Untitled';
            final desc = act['category'] as String? ??
                act['description'] as String? ??
                '...';
              ...
              onTap: () async {
                await Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) => RushInConsumerDetailView(
                            activity: act, onInteraction: () {})));
                _fetch(); // Refresh list on return in case they unsaved
              },
              child: Container(
                margin: const EdgeInsets.only(bottom: 12),
                padding:
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: const Icon(Icons.bookmark,
                        color: Color(0xFF00BFFF), size: 24),
                  ),
                  const SizedBox(width: 16),
                  Expanded(
                      child: Column(
...
class RushInAnalyticsScreen extends StatelessWidget {
  const RushInAnalyticsScreen({super.key});
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          bottom: TabBar(
            ...
            tabs: const [
              Tab(text: 'Hosted by You'),
              Tab(text: 'Joined by You'),
            ],
          ),
        ),
