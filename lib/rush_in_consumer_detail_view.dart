...
class RushInConsumerDetailView extends StatefulWidget {
  ...
  @override
  State<RushInConsumerDetailView> createState() =>
      _RushInConsumerDetailViewState();
}
...
class _RushInConsumerDetailViewState extends State<RushInConsumerDetailView> {
  ...
  Future<void> _loadProfilesForRequests(
  ...
  Future<void> _leaveActivity() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'Are you sure you want to leave this activity? The host will be notified.',
    ...
    try {
      ...
      final notificationText =
          'VACANCY: A participant has left your activity "$activityTitle". Check your waitlist to approve a new member!';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Failed to leave: $e'), backgroundColor: Colors.red));
  ...
  Future<void> _submitJoinRequest() async {
    ...
    try {
      ...
      if (existing != null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('You already requested to join!'),
            backgroundColor: Colors.amber));
      } else {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Join Request Sent! Host will review.'),
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Text('Confirm Deletion',
              style: TextStyle(color: Colors.white)),
          content: const Text(
              'This will notify all participants and permanently delete the activity from everyone\'s feed. Proceed?',
              style: TextStyle(color: Colors.white70)),
          actions: [
            TextButton(
                onPressed: () => Navigator.pop(ctx, false),
    ...
    try {
      if (isHost) {
        ...
        if (participants.isNotEmpty) {
          ...
          final notificationText =
              'ACTIVITY CANCELLED: The host has deleted the activity "$activityTitle".';

          final messagesToInsert = participants
              .map((p) => {
                    'sender_id': _currentUid,
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Activity deleted and participants notified.'),
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Activity removed from your feed.'),
  ...
  Widget _buildAttendeesOverlap(List<Map<String, dynamic>> approvedRequests) {
    ...
    for (int i = 0; i < approvedRequests.length && i < maxShow; i++) {
      ...
      final avatar = req['sender_avatar']?.toString() ??
        ...
        return StatefulBuilder(
          builder: (ctx, setSheet) {
            return StreamBuilder<List<Map<String, dynamic>>>(
              stream: Supabase.instance.client.from('requests').stream(
                  primaryKey: ['id']).eq('target_id', widget.activity['id']),
              builder: (context, snapshot) {
                final requests =
                    snapshot.data ?? [...approvedRequests, ...pendingRequests];
                final currentApproved =
                    requests.where((r) => r['status'] == 'approved').toList();
                final currentPending =
                    requests.where((r) => r['status'] == 'pending').toList();
                _loadProfilesForRequests(
                    [...currentApproved, ...currentPending]);
                                  ...
                                  : ListView.builder(
                                      ...
                                      itemBuilder: (_, i) {
                                        ...
                                        final name = cached?['name'] ??
                                            cached?['full_name'] ??
                                            'User';
                                        String avatar = cached?['avatar_url'] ??
                                            'https://picsum.photos/seed/$sid/200';
          ...
          Container(
            ...
            child: ClipRRect(
              ...
              child: avatar.startsWith('data:')
                  ? Image.memory(base64Decode(avatar.split(',').last),
                      fit: BoxFit.cover)
                  : Image.network(
                      avatar,
                      fit: BoxFit.cover,
          ...
          actions: [
            TextButton(
              ...
              child: Text(
                'OK',
                style:
                    GoogleFonts.plusJakartaSans(color: const Color(0xFFFF7A00)),
              ),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor:
          isDoodleMode(context) ? DoodleColors.cream : const Color(0xFF000000),
      body: StreamBuilder<List<Map<String, dynamic>>>(
          stream: Supabase.instance.client
              .from('requests')
              .stream(primaryKey: ['id']).eq('target_id', act['id']),
          builder: (context, snapshot) {
            final requests = snapshot.data ?? [];

            bool iAmApproved = requests.any((r) =>
                r['sender_id'] == _currentUid && r['status'] == 'approved');
            bool iAmPending = requests.any((r) =>
                r['sender_id'] == _currentUid && r['status'] == 'pending');
              ...
              children: [
                ...
                CustomScrollView(
                  ...
                  slivers: [
                    SliverAppBar(
                      backgroundColor:
                          const Color(0xFF000000), // Solid black when pinned
                      elevation: 0,
                      pinned: true,
                      stretch: true,
                      expandedHeight: 360,
                                  ...
                                  if (isHost || iAmApproved || iAmPending) ...[
                                    StreamBuilder<List<Map<String, dynamic>>>(
                                      ...
                                      builder: (context, chatStatusSnapshot) {
                                        final chatStatusRows =
                                            chatStatusSnapshot.data ?? [];
                    ...
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              (act['category']?.toString() ??
                                      (isRushIn ? 'RUSH-IN' : 'SOCIAL'))
                                  .toUpperCase(),
                              style: GoogleFonts.plusJakartaSans(
                            ...
                            Row(
                              children: [
                                ...
                                isRushIn && targetTime != null
                                    ? _LiveCountdown(
                                        targetTime: targetTime, isTitle: false)
                                    : Text(
                                        isRushIn
                                            ? 'Ends in $remainingStr'
                                            : '$dateStr at $timeStr',
                                        style: GoogleFonts.plusJakartaSans(
                                            color: Colors.white70,
                                            fontSize: 15,
                            ...
                            Builder(builder: (context) {
                              final isGhostMode = act['is_ghost'] == true ||
                                  act['is_ghost_mode'] == true;
                              final bool shouldHideLocation =
                                  isGhostMode && !iAmApproved && !isHost;

                              return GestureDetector(
                                onTap: (!shouldHideLocation)
                                    ? () async {
                                        final lat = act['lat']?.toString() ??
                                            act['latitude']?.toString();
                                        final lng = act['lng']?.toString() ??
                                            act['longitude']?.toString();
                              ...
                              builder: (context, hostSnapshot) {
                                ...
                                final hostAvatar =
                                    host?['avatar_url']?.toString() ??
                                        'https://picsum.photos/seed/host/200';
                                return GestureDetector(
                                  onTap: host != null
                                      ? () => Navigator.push(
                                          context,
                            ...
                            GestureDetector(
                              onTap: () => _showAttendeesModal(
                                  approvedList, waitlist, isHost),
                              child: Row(
                                children: [
                                  Text('Attendees',
                                      style: GoogleFonts.plusJakartaSans(
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                        ...
                                        borderRadius: BorderRadius.circular(8)),
                                    child: Text(
                                        '${approvedList.length + waitlist.length}',
                            ...
                            if (isHost) ...[
                              ...
                              Row(
                                  ...
                                  children: [
                                    const Text('Approved Participants',
                                        style: TextStyle(
                                            color: Colors.white,
                                            fontSize: 16,
                                            fontWeight: FontWeight.bold)),
                                    Text('${approvedList.length}',
                                        style: const TextStyle(
                              ...
                              if (approvedList.isEmpty)
                                const Text('No approved participants yet.',
                                    style: TextStyle(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      return FutureBuilder<
                                          ...
                                          builder: (context, snap) {
                                            ...
                                            final avatar = snap
                                                    .data?['avatar_url'] ??
                                                'https://picsum.photos/seed/${r['sender_id']}/100';
                                            return Container(
                                  ...
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      final name = cached?['name'] ??
                                          cached?['full_name'] ??
                                          'User';
                                      final avatar = cached?['avatar_url'] ??
                                          'https://picsum.photos/seed/$sid/100';
                            ...
                            const SizedBox(
                                height:
                                    120), // Prevent overlap with bottom sticky button
                          ],
                        ),
                      ),
                    ),
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: _buildActionButton(
                      ...
                      onManage: () =>
                          _showAttendeesModal(approvedList, waitlist, isHost),
    ...
    if (imageUrl.startsWith('data:image')) {
      try {
        ...
        return Container(
          decoration: const BoxDecoration(
            gradient:
                LinearGradient(colors: [Color(0xFFFF007F), Color(0xFFFF7E40)]),
          ),
        );
      }
    }
...
class _HostProfileSheetState extends State<_HostProfileSheet> {
  ...
  Future<void> _fetchHostData() async {
    try {
      final p = await Supabase.instance.client
          .from('profiles')
          .select()
          .eq('id', widget.hostUid)
          .maybeSingle();
      final acts = await Supabase.instance.client
          .from('activities')
          .select('id')
          .eq('user_id', widget.hostUid);
      ...
      if (mounted) {
        setState(() {
          ...
          _reputationScore =
              total > 0 ? ((approved / total) * 100).round() : 100;
          _isLoading = false;
        });
      }
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    final avatar = _profile!['avatar_url'] ??
        'https://picsum.photos/seed/${widget.hostUid}/200';
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                CircleAvatar(
                    radius: 36, backgroundImage: _safeImageProvider(avatar)),
              ...
              children: [
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        Text('$_reputationScore%',
                            style: const TextStyle(
                                color: Color(0xFF10B981),
                                fontSize: 24,
                                fontWeight: FontWeight.bold)),
                        const SizedBox(height: 4),
                        const Text('Organizer Rating',
                            style:
                                TextStyle(color: Colors.white54, fontSize: 12),
                            textAlign: TextAlign.center),
                      ],
