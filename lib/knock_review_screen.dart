...
class _KnockReviewScreenState extends State<KnockReviewScreen>
  ...
  void initState() {
    ...
    _pulseCtrl = AnimationController(vsync: this, duration: 1800.ms)
      ..repeat(reverse: true);
  ...
  // ── compat helpers ────────────────────────────────────────────────────────
  Map<String, dynamic> _calcCategories(
      Map<String, dynamic> my, Map<String, dynamic> other) {
    double ls = 0;
    int lsC = 0;
    for (final k in ['smoking', 'drinking', 'diet', 'exercise', 'pets']) {
      final mv = my[k]?.toString() ?? '';
      final ov = other[k]?.toString() ?? '';
      if (mv.isNotEmpty && ov.isNotEmpty) {
        ls += (mv.toLowerCase() == ov.toLowerCase() ? 100 : 20);
        lsC++;
      }
    }
    ...
    double dm = 0;
    int dmC = 0;
    final myAge = (my['age'] as num?)?.toInt() ?? 0;
    final otAge = (other['age'] as num?)?.toInt() ?? 0;
    if (myAge > 0 && otAge > 0) {
      dm += (myAge - otAge).abs() <= 5 ? 100 : 40;
      dmC++;
    }
    for (final k in ['education', 'zodiac']) {
      ...
      final ov = other[k]?.toString() ?? '';
      if (mv.isNotEmpty && ov.isNotEmpty) {
        dm += (mv.toLowerCase() == ov.toLowerCase() ? 100 : 30);
        dmC++;
      }
    }
    ...
    for (final k in ['religion', 'political_view', 'open_to_relocate']) {
      ...
      final ov = other[k]?.toString() ?? '';
      if (mv.isNotEmpty && ov.isNotEmpty) {
        vl += (mv.toLowerCase() == ov.toLowerCase() ? 100 : 30);
        vlC++;
      }
    }
    ...
    final ov = (myI.isEmpty || thI.isEmpty)
        ? 0.0
        : myI.where(thI.contains).length / math.max(myI.length, thI.length);
    final tv = (myT.isEmpty || thT.isEmpty)
        ? 0.0
        : myT.where(thT.contains).length / math.max(myT.length, thT.length);
  ...
  List<String> _calcSharedInterests(
      Map<String, dynamic> my, Map<String, dynamic> other) {
    final myI = List<String>.from(my['interests'] ?? []);
    final thI = List<String>.from(other['interests'] ?? []);
  ...
  List<String> _calcReasons(
    ...
    for (final k in ['smoking', 'diet', 'religion']) {
      ...
      if (mv.isNotEmpty &&
          ov.isNotEmpty &&
          mv.toLowerCase() == ov.toLowerCase()) {
        reasons.add(k == 'smoking'
            ? 'Non-smokers'
            : k == 'diet'
                ? 'Same diet ($mv)'
                : 'Same faith');
  ...
  void _navigateToChat() {
    final senderName = _sender?['name']?.toString() ??
        _sender?['full_name']?.toString() ??
        'User';
  ...
  Widget _buildAmbient() {
    return Positioned.fill(
      child:
          CustomPaint(painter: _KnockAmbientPainter(_accentColor, _pulseCtrl)),
    );
  }
  ...
  Widget _buildProfileCard() {
    final name = _sender?['name']?.toString() ??
        _sender?['full_name']?.toString() ??
        'Anonymous';
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: _accentColor.withValues(alpha: 0.25), width: 1.5),
          boxShadow: [
            BoxShadow(
                  ...
                  child: Column(
                      ...
                      children: [
                        Text('${_compatScore}% Vibe Match',
                            style: GoogleFonts.outfit(
                                color: _scoreColor(_compatScore),
                                fontSize: 13,
                                fontWeight: FontWeight.w700)),
  ...
  Widget _avatarFallback() => Container(
        ...
        child: Icon(Icons.person,
            color: _accentColor.withValues(alpha: 0.6), size: 40),
      );
      ...
      child: AnimatedBuilder(
        ...
        builder: (_, __) => CustomPaint(
          painter: _CompatRingPainter(
              score / 100, _scoreColor(score), _ringCtrl.value),
          child: Center(
            child: Text('$score',
                style: GoogleFonts.outfit(
                    color: Colors.white,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: (isSuper ? _gold : _orange).withValues(alpha: 0.3)),
          ),
          child: Row(children: [
            AnimatedBuilder(
              ...
              child: Column(
                  ...
                  children: [
                    Text(
                      isSuper
                          ? (_revealed
                              ? '$senderName sent you a Super Knock!'
                              : 'Someone sent you a Super Knock!')
                          : (_revealed
                              ? '$senderName wants to connect with you'
                              : 'Someone knocked to connect with you'),
                      style: GoogleFonts.outfit(
                          color: Colors.white,
                          fontSize: 15,
                          fontWeight: FontWeight.w700),
  ...
  Widget _buildAnswersTab() {
    ...
    final hasAnswers = _answers.isNotEmpty &&
        !(_answers.length == 1 &&
            _answers[0] is Map &&
            _answers[0]['super'] == true);

    if (superOnly || !hasAnswers) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.bolt_rounded,
              color: _gold.withValues(alpha: 0.4), size: 60),
          const SizedBox(height: 16),
          ...
          Text(
              'They went straight for the connection!\nAccept to start chatting.',
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                        color: _purple.withValues(alpha: 0.4), blurRadius: 16)
                  ],
                ),
                ...
                Container(
                  ...
                  child: ClipOval(
                    child: (_sender?['avatar_url']?.toString() ?? '').isNotEmpty
                        ? Image.network(_sender!['avatar_url'].toString(),
                            fit: BoxFit.cover,
        ...
        Center(
          child: SizedBox(
            ...
            child: AnimatedBuilder(
              animation: _ringCtrl,
              builder: (_, __) => CustomPaint(
                painter: _CompatRingPainter(_compatScore / 100,
                    _scoreColor(_compatScore), _ringCtrl.value,
                    strokeWidth: 14),
                child: Center(
                  child: Column(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  gradient: _revealed
                      ...
                      : null,
                  color:
                      _revealed ? null : Colors.white.withValues(alpha: 0.05),
                  borderRadius: BorderRadius.circular(16),
                  border: !_revealed
                      ...
                      : Row(
                          ...
                          children: [
                              Icon(
                                  ...
                                  color:
                                      _revealed ? Colors.white : Colors.white24,
                                  size: 16),
                              const SizedBox(width: 8),
                              Text(
...
class _CompatRingPainter extends CustomPainter {
  ...
  _CompatRingPainter(this.progress, this.color, this.animValue,
      {this.strokeWidth = 7});
...
class _KnockAmbientPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Bottom-right orb
    paint.color =
        const Color(0xFF8B5CF6).withValues(alpha: 0.03 + anim.value * 0.015);
    canvas.drawCircle(
        Offset(size.width * 0.85, size.height * 0.85), 160, paint);
  }
