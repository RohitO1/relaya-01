...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void dispose() {
    ...
    locationService.coordinatesUpdateNotifier
  ...
  Future<void> _loadFeed() async {
    ...
    try {
      ...
      if (_activeFilter == 'Trending') {
        ...
        // default local feed (newest first)
        rows = await postQuery.order('created_at', ascending: false).limit(50);
      }
      ...
      // Sort: Followed users first, then by date descending (newest first) (unless Trending)
      if (_activeFilter != 'Trending') {
        postsRows.sort((a, b) {
          ...
          // Secondary sort by date (Descending — latest on top)
  ...
  Future<void> _toggleLike(String postId) async {
    ...
    setState(() {
      ...
      _likeCounts[postId] =
          (_likeCounts[postId] ?? 0) + (alreadyLiked ? -1 : 1);
    });
    try {
      if (alreadyLiked) {
        await _sb
  ...
  Future<void> _loadChats() async {
    ...
    try {
      ...
      if (mounted)
        setState(() {
          _chatList = convos.values.toList();
          _loadingChats = false;
        });
    } catch (e) {
                  ...
                  children: [
                    ...
                    Transform.rotate(
                      ...
                      child: Icon(Icons.send_rounded,
                          color: DoodleColors.sketchLineLight, size: 20),
                    ),
                  ],
                )
              : Text(
              ...
              GestureDetector(
                ...
                child: SizedBox(
                  ...
                  child: Icon(Icons.search,
                      color: doodle ? DoodleColors.textSecondary : Colors.white,
                      size: 28),
                ),
              ),
              const SizedBox(width: 8),
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      gradient: LinearGradient(
                          colors: [HomeColors.pink, HomeColors.orange]),
                      border: Border.all(color: HomeColors.bg, width: 1.5),
                      boxShadow: [
                        BoxShadow(
                            color: HomeColors.pink.withValues(alpha: 0.6),
                            blurRadius: 6)
                      ],
                    ),
                  )
                  ...
                  onTap: () {
                    if (postUserId.isNotEmpty) {
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (_) =>
                                  ProfileScreen(userId: postUserId)));
                    }
                  },
                  child: GestureDetector(
                    onLongPress: postAvatarUrl.isNotEmpty
                        ? () =>
                            _showPhotoPreview(context, postAvatarUrl, initial)
                        : null,
                    child: Container(
                      width: 44,
                      height: 44,
                      decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        gradient: postAvatarUrl.isEmpty
                            ? LinearGradient(
                      ...
                      child: ClipOval(
                          child: _buildAvatarWidget(postAvatarUrl, initial)),
                    ),
                  ),
                ),
                  ...
                  children: [
                    ...
                    Row(children: [
                      Text(_timeAgo(createdAt),
                          style: GoogleFonts.inter(
                              fontSize: 11, color: HomeColors.muted)),
                      if (city.isNotEmpty) ...[
            ...
            child: Row(
              children: [
                _buildActionBtn(
                    ...
                    '$likeCount',
                    isLiked ? HomeColors.pink : HomeColors.txt2,
                    () => _toggleLike(postId)),
  ...
  Widget _buildPostText(String text) {
    return _ExpandablePostText(text: text);
  }

  Widget _buildAvatarWidget(String avatarUrl, String initial) {
            ...
            child: Center(
              child: ClipOval(
                child: SizedBox(
                  width: 280,
                  height: 280,
                  child: _buildAvatarWidget(avatarUrl, initial),
                ),
              ),
            ),
          ),
  ...
  Widget _buildActionBtn(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: color.withValues(
              alpha:
                  label.isNotEmpty && color != HomeColors.muted ? 0.12 : 0.0),
        ),
        child: Row(
          children: [
      ...
      ).animate(target: color != HomeColors.muted ? 1 : 0).scale(
  ...
  Widget _buildMoreOption(IconData icon, String label, VoidCallback onTap,
    ...
    return ListTile(
      leading:
          Icon(icon, color: isRed ? HomeColors.red : HomeColors.txt2, size: 20),
      title: Text(label,
          style: GoogleFonts.inter(
              fontSize: 13,
              fontWeight: FontWeight.w500,
              color: isRed ? HomeColors.red : HomeColors.txt2)),
      onTap: onTap,
                ...
                child: Row(children: [
                  Expanded(
                    ...
                    style: doodle
                        ...
                        : GoogleFonts.inter(
                            fontSize: 13, color: HomeColors.txt),
                    decoration: InputDecoration(
                      ...
                      hintStyle: doodle
                          ? DoodleFonts.body(
                              ...
                              fontSize: 16)
                          : GoogleFonts.inter(color: HomeColors.muted),
                  ...
                  IconButton(
                    icon: Icon(Icons.send,
                        color: doodle ? DoodleColors.blue : HomeColors.cyan,
                        size: 20),
                    onPressed: () {
                                ...
                                Container(
                                  decoration: doodle
                                      ...
                                      : BoxDecoration(
                                          ...
                                          border:
                                              Border.all(color: HomeColors.gb),
                                        ),
                                  child: TextField(
                                    controller: textCtrl,
                                    maxLines: 5,
                                        ...
                                        builder: (ctx) {
                                          ...
                                          final profileState =
                                              (_myProfile?['state'] ?? '')
                                                  .toString()
                                                  .trim();
                                          final svcDistrict = locationService
                                              .activeDistrict
                                              .trim();
                                          final svcState = locationService
                                              .activeState
                                              .trim();
                                          final String dist =
                                          ...
                                          final String st =
                                              profileState.isNotEmpty
                                                  ? profileState
                                                  : svcState;
                                          if (dist.isEmpty && st.isEmpty)
                                            return const SizedBox.shrink();
                                          final displayText = [dist, st]
                                                ...
                                                try {
                                                  ...
                                                  final svcDist =
                                                      locationService
                                                          .activeDistrict
                                                          .trim();
                                                  final svcSt = locationService
                                                      .activeState
                                                      .trim();
                                                  final profDist = (_myProfile?[
                                                              'district'] ??
                                                          '')
                                                      .toString()
                                                      .trim();
                    ...
                    child: ListView(
                      ...
                      children: [
                        _buildNotifItem(
                            ...
                            HomeColors.orange,
                            'Rush-in Accepted! ⚡',
                            'Your request for "Late Night Coffee" has been accepted.',
                            '2m ago',
                            true),
                        _buildNotifItem(
                            ...
                            HomeColors.cyan,
                            'Activity Approved 🎉',
                            'You\'ve been approved to join "Weekend Trek to Rajmachi"',
                            '15m ago',
                            true),
                        _buildNotifItem(
                            Icons.star,
                            HomeColors.purple,
                            'New AI Match! 💫',
                            'You have a new high-compatibility match based on your interests.',
                            '1h ago',
                            true),
                        _buildNotifItem(
                            Icons.favorite,
                            HomeColors.pink,
                            'Event Reminder',
                            'Concert Night starts in 3 hours. Don\'t forget!',
                            '2h ago',
                            false),
                        _buildNotifItem(
                            Icons.person_add,
                        ...
                        _buildNotifItem(
                            ...
                            HomeColors.blue,
                            'Waitlist Update',
                            'A spot opened for "Street Food Crawl". Claim it now!',
                            '5h ago',
                            false),
                      ],
                    ),
  ...
  Widget _buildNotifItem(IconData icon, Color color, String title, String desc,
      String time, bool unread) {
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(12),
        color: unread
            ? HomeColors.cyan.withValues(alpha: 0.03)
            : Colors.transparent,
        border: Border.all(
            color: unread
                ? HomeColors.cyan.withValues(alpha: 0.1)
                : Colors.transparent),
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Expanded(
              child: Column(
                ...
                Text(desc,
                    style: GoogleFonts.inter(
                        fontSize: 11, color: HomeColors.muted, height: 1.4)),
              ])),
          Text(time,
              style: GoogleFonts.inter(fontSize: 10, color: HomeColors.muted)),
        ],
      ),
    );
  }
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                            child: TextField(
                          style: GoogleFonts.inter(
                              fontSize: 12, color: HomeColors.txt),
                          decoration: InputDecoration(
    ...
    if (_loadingChats)
      return Center(
          child: CircularProgressIndicator(
              strokeWidth: 2, color: HomeColors.cyan));
    if (_chatList.isEmpty)
      return Center(
          child: Text('No messages yet',
              style: GoogleFonts.inter(color: HomeColors.muted, fontSize: 13)));
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
              border: Border(bottom: BorderSide(color: HomeColors.gb))),
          child: Row(children: [
            Container(
              width: 44,
              height: 44,
                ...
                child: Column(
                  ...
                  Row(
                      ...
                      children: [
                        ...
                        Text(time,
                            style: GoogleFonts.inter(
                                fontSize: 10, color: HomeColors.muted)),
                      ]),
                  const SizedBox(height: 2),
                  Text(msg,
                      style: GoogleFonts.inter(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                gradient: LinearGradient(colors: [
                  HomeColors.purple.withValues(alpha: 0.2),
                  HomeColors.pink.withValues(alpha: 0.2)
                ]),
              ),
              child: Center(child: Text('🔒', style: TextStyle(fontSize: 28))),
            ),
            const SizedBox(height: 16),
            Text('Unlock Full Profiles',
                style: GoogleFonts.inter(
                    fontSize: 18,
                    fontWeight: FontWeight.w700,
                    color: HomeColors.txt)),
            const SizedBox(height: 6),
            Text(
                'Subscribe to Meetra Premium to view profiles, send messages, and connect with people from the feed.',
                style: GoogleFonts.inter(
                    fontSize: 12, color: HomeColors.muted, height: 1.5),
                textAlign: TextAlign.center),
            ...
            ...[
              ...
              ['Priority event bookings', Icons.check_circle],
                  ...
                  child: Row(children: [
                    ...
                    Text(f[0] as String,
                        style: GoogleFonts.inter(
                            fontSize: 12, color: HomeColors.txt2)),
                  ]),
                )),
            const SizedBox(height: 20),
            Row(children: [
              ...
              Expanded(
                ...
                onTap: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text('Redirecting to subscription...')));
                },
                child: Container(
                  padding: const EdgeInsets.symmetric(vertical: 12),
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(12),
                    gradient: LinearGradient(
                        colors: [HomeColors.purple, HomeColors.pink]),
                  ),
                  child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              border: Border.all(
                  color: HomeColors.cyan.withValues(alpha: 0.3), width: 1.5),
            ),
          )
              .animate(onPlay: (c) => c.repeat(reverse: true))
              .scale(
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                    color: HomeColors.cyan.withValues(alpha: 0.25),
                    blurRadius: 20,
                    offset: Offset(0, 4)),
              ],
            ),
          ...
          children: List.generate(
              ...
              (_) => Container(
                    ...
                    child: Column(
                      ...
                      .animate(onPlay: (c) => c.repeat(reverse: true))
                      .fade(begin: 0.3, end: 0.6, duration: 1500.ms))),
    );
  }
...
class _FilterBarDelegate extends SliverPersistentHeaderDelegate {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Row(
              children: [
                _buildFilterChip('Trending', activeFilter == 'Trending',
                    () => onFilterChanged('Trending'), doodle),
                const SizedBox(width: 6),
                GestureDetector(
                  onTap: onLocationTap,
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color:
                          doodle ? DoodleColors.paper : const Color(0xFF1E1E1E),
                      shape: BoxShape.circle,
                      border: Border.all(
                          color:
                              doodle ? DoodleColors.cardBorder : Colors.white24,
                          width: 1),
                    ),
  ...
  Widget _buildFilterChip(
      String label, bool isActive, VoidCallback onTap, bool doodle) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
          ...
          Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    colors: [
                      HomeColors.purple.withValues(alpha: 0.04),
                      Colors.transparent
                    ],
                    stops: const [0.0, 1.0],
                  ),
          ...
          Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    colors: [
                      HomeColors.pink.withValues(alpha: 0.03),
                      Colors.transparent
                    ],
                    stops: const [0.0, 1.0],
                  ),
...
class _MeetraMLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    outer.cubicTo(
        x0, yMid + sw, x1 - sw, yTop + sw, x1, yTop); // left arch outer
    outer.cubicTo(x1 + sw * 0.5, yTop, x2 - sw, yMid - sw * 0.3, x2,
        yMid); // left leg to valley outer
    outer.cubicTo(x2 + sw, yMid - sw * 0.3, x3 - sw * 0.5, yTop, x3,
        yTop); // valley to right leg outer
    outer.cubicTo(
        x3 + sw, yTop + sw, x4, yMid + sw, x4, yBot); // right arch outer
    outer.lineTo(x4 - sw * 2, yBot);

    // inner path (counter-clockwise inner edge of M)
    final innerRightFoot = Offset(x4 - sw * 2, yBot);
    outer.cubicTo(innerRightFoot.dx, yMid + sw * 1.5, x3 + sw * 0.3,
        yTop + sw * 2, x3, yTop + sw * 2); // right arch inner
    outer.cubicTo(x3 - sw * 0.3, yTop + sw * 2, x2 + sw, yMid + sw * 0.5, x2,
        yMid + sw * 1.5); // right leg inner
    outer.cubicTo(x2 - sw, yMid + sw * 0.5, x1 + sw * 0.3, yTop + sw * 2, x1,
        yTop + sw * 2); // left leg inner
    outer.cubicTo(x1 - sw * 0.3, yTop + sw * 2, x0 + sw * 2, yMid + sw * 1.5,
        x0 + sw * 2, yBot); // left arch inner
    outer.close();
...
class _ExpandablePostText extends StatefulWidget {
  final String text;
  const _ExpandablePostText({Key? key, required this.text}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    for (final match in pattern.allMatches(widget.text)) {
      ...
      spans.add(TextSpan(
        text: tag,
        style: TextStyle(
            color: tag.startsWith('#') ? HomeColors.cyan : HomeColors.purple,
            fontWeight: FontWeight.w500),
      ));
      last = match.end;
    }
    ...
    final textSpan = TextSpan(
      style:
          GoogleFonts.inter(fontSize: 13, height: 1.6, color: HomeColors.txt),
      children: spans,
    );
