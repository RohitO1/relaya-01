...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  @Nullable ConstraintsMap androidAudioConfiguration, Severity logSeverity, @Nullable Integer audioSampleRate, @Nullable Integer audioOutputSampleRate) {
    ...
    // ── VoiceMask integration ──────────────────────────────────────────
    // Set our bridge as the AudioBufferCallback BEFORE creating the module.
    // This is the ONLY reliable way to intercept raw PCM audio because
    // WebRtcAudioRecord.audioBufferCallback is a 'private final' field —
    // it can only be set at construction time via the Builder.
    audioDeviceModuleBuilder.setAudioBufferCallback(new VoiceMaskBridge());
    Log.i(TAG, "VoiceMaskBridge set as AudioBufferCallback on builder");

    // FORCE bypassVoiceProcessing to true to disable hardware AEC/NS
    // This ensures WebRTC uses the standard Java AudioRecord path and doesn't
    // bypass our AudioBufferCallback with a C++ hardware module.
    audioDeviceModuleBuilder.setUseHardwareAcousticEchoCanceler(false)
                            .setUseHardwareNoiseSuppressor(false)
                            .setUseStereoInput(true)
                            .setUseStereoOutput(true)
